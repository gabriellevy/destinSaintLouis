

label intro:
    scene bg armee_franque
    with dissolve
    show screen valeurs_traits
    "Vous êtes Louis IX mais vous êtes encore un enfant."
    scene bg louis_8
    "Loin d'être Saint Louis ou même Louis IX, vous êtes encore surnommé Louis de Poissy, par votre lieu de naissance. Car le roi Louis de France est pour l'instant Louis VIII, votre père."
    "Auréolé de sa victoire contre l'anglais Jean sans terre, Louis le lion trône fièrement aux côté de Blanche de Castille, votre mère."
    scene bg louis_et_sa_mere
    "Blanche est une femme à la fois forte, courageuse et ambitieuse."
    "Si on rajoute le fait qu'elle est étrangère il est facile de comprendre qu'elle est peu populaire parmi les courtisans."
    "Cela ne l'empêche pas de tenir une place de premier ordre dans les cercles du pouvoir. Surtout quand son valeureux époux part guerroyer d'un bout à l'autre de son royaume."
    jump fin_cycle
